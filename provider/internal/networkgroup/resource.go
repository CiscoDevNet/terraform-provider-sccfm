...
// OverrideModel represents a per-target override for a network group.
...
// ResourceModel is the Terraform state model for a network group.
...
func NewResource() resource.Resource {
	return objectresource.NewObjectResource(objectresource.ObjectResourceConfig[ResourceModel]{
		...
		MarkdownDescription: "Provides a Network Group resource. Use this resource to create, read, update, and delete network groups in the Security Cloud Control unified object store. A network group can contain inline network values and/or references to existing network objects.",
		ExtraSchemaAttributes: func() map[string]schema.Attribute {
			return map[string]schema.Attribute{
				"values": schema.ListAttribute{
					MarkdownDescription: "Inline network values (IP addresses, CIDR blocks, or ranges) included in the group.",
					Optional:            true,
				...
				"overrides": schema.ListNestedAttribute{
					...
					NestedObject: schema.NestedAttributeObject{
						Attributes: map[string]schema.Attribute{
							...
							"values": schema.ListAttribute{
								MarkdownDescription: "Inline network values for this target override.",
								Optional:            true,
								ElementType:         types.StringType,
							},
							"referenced_object_uids": schema.SetAttribute{
								MarkdownDescription: "Set of UIDs of network objects for this target override.",
								Optional:            true,
								ElementType:         types.StringType,
							},
						},
...
func marshalNetworkLiterals(values []types.String) ([]json.RawMessage, error) {
	var literals []json.RawMessage
	for _, v := range values {
		raw, err := json.Marshal(object.NetworkContent{Literal: v.ValueString()})
		if err != nil {
			return nil, err
		}
...
func buildGroupContent(model *ResourceModel) (*object.GroupContent, error) {
	...
	if !model.Values.IsNull() && !model.Values.IsUnknown() {
		...
		literals, err := marshalNetworkLiterals(vals)
		if err != nil {
			return nil, err
		}
...
func buildOverrides(overrides []OverrideModel) ([]object.Override, error) {
	...
	for _, o := range overrides {
		gc := &object.GroupContent{}
		literals, err := marshalNetworkLiterals(o.Values)
		if err != nil {
			return nil, err
		}
		gc.Literals = literals
...
func unmarshalNetworkLiterals(literals []json.RawMessage) ([]string, error) {
	var values []string
	for _, raw := range literals {
		var nc object.NetworkContent
		if err := json.Unmarshal(raw, &nc); err != nil {
			return nil, err
		}
		values = append(values, nc.Literal)
	}
	return values, nil
}
...
func mapReadOutput(ctx context.Context, output *object.ReadOutput, model *ResourceModel) error {
	...
	values, err := unmarshalNetworkLiterals(gc.Literals)
	if err != nil {
		return err
	}
	if len(values) > 0 {
	...
	for _, o := range output.Value.Overrides {
		...
		oValues, err := unmarshalNetworkLiterals(ogc.Literals)
		if err != nil {
			return err
		}
